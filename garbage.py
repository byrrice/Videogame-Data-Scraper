...
def collect_data(start, end):
    ...
    for i in range(start, end + 1):
        ...
        # print(names)
        # print(temp_comp)
        # name_str = str(init_name).split("\n")
        # # name_str = list(filter(None, str(init_name).split("\n")))
        # init_rating = soup.find_all('b')
        # rating_str = str(init_rating).split(",")[1:]
        # # print(init_name)
        # print(len(rating_str), len(name_str))
        # #again getting all the requisite data
        # for j in range(0, len(rating_str)):
        #     modified_str = re.sub('[^\d\.]', '', rating_str[j])
        #     rating.append(modified_str)
        #     name.append(find_between(name_str[j], "\">", "</a>"))
        #     company.append(find_between(name_str[j], "\t\t", "\t\t"))
...
def write_data():
    ...
    #final touches to all array
    # rating = list(filter(None, rating))
    # name = list(filter(None, name))
    # company = list(filter(None, company))

    # separating dates and companies
    # for k in range(0, len(company)):
    #
    #     #NEED TO FIX IN ORDER TO ONLY GET LAST 4 DIGITS
    #     date.append(re.sub('[^\d]', '', company[k])[-4:])
    #     company[k] = re.sub(r'[^a-zA-Z\/ ]+', '', company[k])
...
# unimplemented graphing method which will use threading to graph our data
#def graph_data(data):
